...
# Export FreeSurfer environment inside the script (per your preference)
...
# run recon-all with the provided directives and any extra flags
...
echo "=== Running antsRegistration to standard space ==="
antsRegistration \
  --dimensionality 3 \
  --float 0 \
  --output "[${SUBJECTS_DIR}/${SUBJID}/mri/aseg_MNI_,${SUBJECTS_DIR}/${SUBJID}/mri/aseg_MNI_Warped.nii.gz]" \
  --interpolation nearestNeighbor \
  --winsorize-image-intensities [0.005,0.995] \
  --use-histogram-matching 0 \
  --initial-moving-transform "[${STD_SPACE},${ASEG_NII},1]" \
  --transform Rigid[0.1] \
  --metric "MI[${STD_SPACE},${ASEG_NII},1,32,Regular,0.25]" \
  ...
  --metric "CC[${STD_SPACE},${ASEG_NII},1,4]" \
