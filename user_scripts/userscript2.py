...
def create_correlation_figures(df_tidy, output_dir):
    ...
    for dataset in df_tidy['dataset'].unique():
        ...
        for i, structure in enumerate(structures):
            ...
            pivot_df = structure_data.pivot(index='subject', columns='pipeline_short', values='volume_mm3')
            ...
            # Correct n: number of subjects per structure (same for all pipelines)
        ...
        sm.set_array([])
        fig.colorbar(sm, cax=cbar_ax).set_label("Pearson correlation", rotation=270, labelpad=15)
