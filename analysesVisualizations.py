...
def boxplot(volume_list, pipeline_name, dataset_name):
    ...
    plt.savefig(
        "./artifacts/" + dataset_name + "_" + pipeline_name + "_box" + ".png"
    )  # Saves in artifact directory
    # plt.show()
...
def corrplot(volume_list, hearing_loss_list, pipeline_name, dataset_name):
    ...
    plt.ylim(ymin=0)
    plt.title(
        "Left Hippocampal Volumes vs Worse_ear_dsi"
        + "\n"
        + dataset_name
        + " with "
        + pipeline_name
    ...
    plt.savefig(
        "./artifacts/" + dataset_name + "_" + pipeline_name + "_corr" + ".png"
    )  # Saves in artifact directory
        ...
        for entry in cache:
            ...
            if cache[entry][pipeline_name]["Result"]["result"] != None:
                ...
                if (
                    ...
                    if (
                        hearing_loss != None
                    ):  # only visualize if we have a hearing loss measure for subject/visit
                        hearing_loss_list.append(hearing_loss)
                        volume_list.append(volume)

    if (
        len(volume_list) >= 1 and len(hearing_loss_list) >= 1
...
cbrain_download_file(
    3497558, preventAD_data_file, cbrain_token
