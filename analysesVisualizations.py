...
def boxplot(volume_list, pipeline_name, dataset_name):
    ...
    plt.savefig('./artifacts/' + dataset_name + '_' +
                pipeline_name + '_box' + '.png')  # Saves in artifact directory
    # plt.show()
...
def corrplot(volume_list, hearing_loss_list, pipeline_name, dataset_name):
    ...
    plt.savefig('./artifacts/' + dataset_name + '_' + pipeline_name +
                '_corr' + '.png')  # Saves in artifact directory
...
def preventAD_process(data_file, cache_file, pipeline_name):
    ...
    with open(cache_file, "r") as file:
        ...
        for entry in cache:
            ...
            if cache[entry][pipeline_name]['Result']['result'] != None:
                ...
                # If there is more than one word in the result string - necessary for FSL, but maybe not for other pipelines in future.
                if volume != 1:
                    volume = volume.partition(' ')[0]  # Get the first word
                    subject, visit = preventAD_get_labels_from_filename(entry)

                    try:
                        hearing_loss = preventAD_get_measure_from_csv(
                            subject, visit, data_file)
    ...
    if len(volume_list) >= 1 and len(hearing_loss_list) >= 1:
...
cbrain_api = cbrainAPI.CbrainAPI(cbrain_user, cbrain_password)
...
# use this (quicker) if you know the CBRAIN userfileID
cbrain_api.download_file(3497558, preventAD_data_file)
